...
- `memories/repo/` files — regenerate module indexes, symbol maps, API route tables, third-party API contracts, data model references, query catalogs, env-var references, edge-function catalogs, project maps, and `knowledge-packs.md` when requirement-pack policy or skill inventory changed. Use descriptive filenames so agents know when to read them.
   - **`memories/repo/project-profile.md` — the `<!-- digest:start -->`/`<!-- digest:end -->` block is THE always-loaded context injected into every session for this project.** Any change to features, file layout, key symbols, project lexicon, in-flight/half-done state, or destructive-action invariants (grace windows, irreversible steps) MUST be reflected here — otherwise the next session re-derives it by searching. Keep it self-sufficient across all six dimensions (what/stack, feature→path map, key symbols, lexicon/glossary, pending/half-done, hard rules) and within the ~1,500–2,500-token budget defined in the `consolidate-project` skill's "Digest standard". If a term or a WIP gap would force a fresh agent to grep, it belongs in the digest.
