...
- **Built registry**: generated JSON files served to CLI consumers, usually
  from `public/r`. Use `npx shadcn@4.12.0 build` to create this form.
...
```bash
npx shadcn@4.12.0 build
npx shadcn@4.12.0 build registry.json --output public/r
```

Use CLI commands to inspect the result:

```bash
npx shadcn@4.12.0 list @acme
npx shadcn@4.12.0 search @acme -q "login"
npx shadcn@4.12.0 view @acme/login-form
npx shadcn@4.12.0 add @acme/login-form --dry-run
npx shadcn@4.12.0 registry validate ./registry.json
```

Use GitHub addresses directly for public GitHub registries:

```bash
npx shadcn@4.12.0 list owner/repo
...
npx shadcn@4.12.0 registry validate owner/repo
```
